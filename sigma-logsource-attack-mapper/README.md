...
Parses [Sigma](https://github.com/SigmaHQ/sigma) rule YAML files and produces aggregate analytics for logsource usage, tag usage, MITRE ATT&CK technique coverage, and technique-to-logsource mappings.

Useful for understanding what TTPs and data sources appear most frequently in a detection rule set — which can inform log source value, TTP prevalence, etc.

Works natively with the [official SigmaHQ/sigma repo](https://github.com/SigmaHQ/sigma) via `--sigma-repo`, or against any directory of YAML rule files.
